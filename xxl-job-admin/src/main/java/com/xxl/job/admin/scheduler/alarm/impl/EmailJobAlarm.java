...
import com.xxl.job.admin.scheduler.alarm.msg.EmailMsg;
...
public class EmailJobAlarm extends AbstractJobAlarm {
    ...
    private String getAlarmContent(XxlJobLog jobLog) {
        ...
        if (jobLog.getHandleCode() > 0 && jobLog.getHandleCode() != ResponseCode.SUCCESS.getCode()) {
    ...
    public boolean sendToAll(XxlJobLog jobLog, XxlJobInfo info, String jsonMsg) {
        try {
            EmailMsg emailMsg = GsonTool.fromJson(jsonMsg, EmailMsg.class);
            ...
            helper.setFrom(XxlJobAdminBootstrap.getInstance().getEmailFrom(), I18nUtil.getString("admin_name_full"));
